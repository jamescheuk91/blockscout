defmodule Explorer.Chain.Address.TokenBalance do
  ...
  * Excluding the burn address (0x0000000000000000000000000000000000000000).
  ...
  defp page_token_balances(query, %PagingOptions{key: {value, block_number}}) do
    where(
      query,
      [tb],
      tb.value < ^value or (tb.value == ^value and tb.block_number > ^block_number) or
        (tb.value == ^value and tb.block_number < ^block_number)
    )
  end
end
